...
class WordPair:
    def __init__(self, max_sequence_len=512):
        ...
        # self.entity_dic = {"O": 0, "ENT-Tri": 1, "ENT-Arg": 2, "ENT-Opi-Pos": 3, 'ENT-Opi-Neg': 4, 'ENT-Opi-Neu': 5}
    ...
    def encode_entity(self, elements, entity_type='ENT-Tri'):
        ...
        """
        # if entity_type == 'ENT-Tri' or entity_type == 'ENT-Arg':
        entity_list = []
        for line in elements:
            start, end = line[:2]
            entity_list.append((start, end, self.entity_dic[entity_type]))
        # else:
        #     dic = {'positive': 'ENT-Opi-Pos', 'negative': 'ENT-Opi-Neg', 'neutral': 'ENT-Opi-Neu'}
        #     entity_list = []
        #     for line in elements:
        #         start, end = line[:2]
        #         entity_list.append((start, end, self.entity_dic[dic[line[-1]]]))
        return entity_list
    ...
    def encode_relation(self, quadruples):
        ...
        for quad in quadruples:
            ...
            #         arg_start, arg_end = arg
            #         if arg_start != -1:
            #             rel_list.append((opinion_start, arg_start, self.rel_dic['h2h']))
            #             rel_list.append((opinion_end, arg_end, self.rel_dic['t2t']))
        return rel_list
    ...
    def encode_sentiment(self, quadruples):
        ...
        sentiment_list = []
        # for quad in quadruples:
        #     _, _, _, opinion_start, opinion_end, sentiment = quad
        #     sentiment_list.append((opinion_start, opinion_end, sentiment))
        for quad in quadruples:
    ...
    def get_quadruples(self, ent_matrix, rel_matrix, pol_matrix, new_sentences_ids):
        ...
        # res, pair = self.decode_quadruple(ent_list, rel_list, new_sentences_ids)
    ...
    def decode_quadruple(self, ent_list, rel_list, pol_list, new_sentences_ids):
        ...
        #             if (*subj, *third) not in quadruple_set: continue
        ...
        # pairs = {'trigger-arg': [], 'trigger-opinion': [], 'arg-opinion': []}
        ...
        for line in quadruple_set:
            ...
            # elif tp1 == 2 and tp2 in [3, 4, 5]:
            #     # pairs['arg-opinion'].append(line)
            #     continue
            elif tp1 == 1 and tp2 == 3:
                pairs['trigger-opinion'].append(line)
            elif tp2 == 1 and tp1 == 2:
                pairs['trigger-arg'].append((h2, t2, h1, t1))
            # elif tp2 == 2 and tp1 in [3, 4, 5]:
            #     # pairs['arg-opinion'].append((h2, t2, h1, t1))
            #     continue
            elif tp2 == 1 and tp1 == 3:
                pairs['trigger-opinion'].append((h2, t2, h1, t1))
        # return set(tetrad), pairs

        # pick up opinions according to entity2type, if entity2type is 3/4/5, then it is an opinion
        # opinion_senti_pairs = []
        # for line in quadruples:
        #     h1, t1, h2, t2 = line
        #     tp1 = entity2type[(h1, t1)]
        #     tp2 = entity2type[(h2, t2)]
        #     if tp1 in [3, 4, 5]:
        #         opinion_senti_pairs.append((h1, t1, tp1))
        #     if tp2 in [3, 4, 5]:
        #         opinion_senti_pairs.append((h2, t2, tp2))
                # build (trigger, opinion, sentiment) triplets
        tri_opi_senti_pairs = []
        for ele1_s, ele1_e, ele2_s, ele2_e in pairs['trigger-opinion']:
            # senti = b2b_relation_set.get((ele2_s, ele2_e), 2)
            # tri_opi_senti_pairs.append((ele1_s, ele1_e, ele2_s, ele2_e, senti))
            ...
            # else:
            #     tri_opi_senti_pairs.append((ele1_s, ele1_e, ele2_s, ele2_e, 2))
