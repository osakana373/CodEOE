...
class Run_eval:
    ...
    def compute_score(self, mode='quad'):
        ...
        for doc_id in self.gold_res:
            ...
            if mode != 'iden' and mode != 'quad':
                fp += len(set(pred_line) - set(gold_line))
                fn += len(set(gold_line) - set(pred_line))
                tp += len(set(pred_line) & set(gold_line))
            else:
                # for quad and iden, we need to compare the whole list
                tmp_tp = 0
                for pred in pred_line:
                    for gold in gold_line:
                        # compare the trigger, opinion and sentiment
                        if gold[:2] != pred[:2]:
                            continue
                        if gold[3:5] != pred[3:5]:
                            continue
                        if mode == 'quad' and gold[-1] != pred[-1]:
                            continue
                        tmp_tp += 1
                tp += tmp_tp
    ...
    def compute_partial_matching_score_for_text1(self, mode='triggers', lang='zh'):
        tp_chars, fp_chars, fn_chars = 0, 0, 0
        tri_tp_chars, tri_fp_chars, tri_fn_chars = 0, 0, 0
        else_tp_chars, else_fp_chars, else_fn_chars = 0, 0, 0
        sentiment_tp_chars, sentiment_fp_chars, sentiment_fn_chars = 0, 0, 0
            ...
            if mode in ['triggers', 'arguments', 'opinions']:
                for pred in pred_line:
                    ...
                    for gold in gold_line:
                        ...
                        if matching_chars > max_matching_chars or (max_matching_chars == matching_chars and len(gold) < len(true_gold)):
                            max_matching_chars = matching_chars
                            true_gold = gold
                    tp_chars += max_matching_chars
                    fp_chars += len(pred) - max_matching_chars
                    fn_chars += len(true_gold) - max_matching_chars  

            elif mode in ['trigger-arg', 'trigger-opinion']:
                # 对于trigger-arg, trigger-opinion, 每个pred有两个str
                for pred in pred_line:
                    trigger_max_matching_chars = 0
                    else_max_matching_chars = 0
                    trigger_true_gold = ''
                    else_true_gold = ''
                    pred_trigger, pred_else = pred
                    ...
                    for gold in gold_line:
                        ...
                        if lang == 'zh':
                            trigger_matching_chars = self.count_matching_chars(pred_trigger, gold_trigger)
                            else_matching_chars = self.count_matching_chars(pred_else, gold_else)
                            score = (trigger_matching_chars / len(gold_trigger) + else_matching_chars / len(gold_else)) / 2
                        elif lang == 'en':
                            trigger_matching_chars = self.count_matching_chars(pred_trigger.split(), gold_trigger.split())
                            else_matching_chars = self.count_matching_chars(pred_else.split(), gold_else.split())
                            score = (trigger_matching_chars / len(gold_trigger.split()) + else_matching_chars / len(gold_else.split())) / 2 

                        if score > max_matching_score or (lang == 'zh' and max_matching_score == score and len(gold_trigger) + len(gold_else) < len(trigger_true_gold) + len(else_true_gold)) \
                            or (lang == 'en' and max_matching_score == score and len(gold_trigger.split()) + len(gold_else.split()) < len(trigger_true_gold.split()) + len(else_true_gold.split())):
                            max_matching_score = score
                            trigger_max_matching_chars = trigger_matching_chars
                            trigger_true_gold = gold_trigger
                            else_max_matching_chars = else_matching_chars
                            else_true_gold = gold_else

                    if lang == 'en':
                        pred_trigger = pred_trigger.split()
                        pred_else = pred_else.split()
                        trigger_true_gold = trigger_true_gold.split()
                        else_true_gold = else_true_gold.split()

                    tri_tp_chars += trigger_max_matching_chars
                    tri_fp_chars += len(pred_trigger) - trigger_max_matching_chars
                    tri_fn_chars += len(trigger_true_gold) - trigger_max_matching_chars
                    else_tp_chars += else_max_matching_chars
                    else_fp_chars += len(pred_else) - else_max_matching_chars
                    else_fn_chars += len(else_true_gold) - else_max_matching_chars
            else:
                for pred in pred_line:
                    trigger_max_matching_chars = 0
                    else_max_matching_chars = 0
                    sentiment_max_matching_chars = 0
                    trigger_true_gold = ''
                    else_true_gold = ''
                    sentiment_true_gold = ''
                    pred_sentiment, pred_trigger, pred_opi = pred
                    ...
                    for gold in gold_line:
                        ...
                        if lang == 'zh':
                            trigger_matching_chars = self.count_matching_chars(pred_trigger, gold_trigger)
                            else_matching_chars = self.count_matching_chars(pred_opi, gold_opi)
                            score = (trigger_matching_chars / len(gold_trigger) + else_matching_chars / len(gold_opi) + sentiment_matching_chars) / 3

                        elif lang == 'en':
                            trigger_matching_chars = self.count_matching_chars(pred_trigger.split(), gold_trigger.split())
                            else_matching_chars = self.count_matching_chars(pred_opi.split(), gold_opi.split())
                            score = (trigger_matching_chars / len(gold_trigger.split()) + else_matching_chars / len(gold_opi.split()) + sentiment_matching_chars) / 3

                        if score > max_matching_score or (lang == 'zh' and max_matching_score == score and len(gold_trigger) + len(gold_opi) < len(trigger_true_gold) + len(else_true_gold) and sentiment_matching_chars == 1) \
                            or (lang == 'en' and max_matching_score == score and len(gold_trigger.split()) + len(gold_opi.split()) < len(trigger_true_gold.split()) + len(else_true_gold.split()) and sentiment_matching_chars == 1):
                            max_matching_score = score
                            trigger_max_matching_chars = trigger_matching_chars
                            trigger_true_gold = gold_trigger
                            else_max_matching_chars = else_matching_chars
                            else_true_gold = gold_opi
                            sentiment_max_matching_chars = sentiment_matching_chars
                            sentiment_true_gold = gold_sentiment
                    
                    if lang == 'en':
                        pred_trigger = pred_trigger.split()
                        pred_opi = pred_opi.split()
                        pred_sentiment = pred_sentiment.split()
                        trigger_true_gold = trigger_true_gold.split()
                        else_true_gold = else_true_gold.split()
                        sentiment_true_gold = sentiment_true_gold.split()
                        
                    tri_tp_chars += trigger_max_matching_chars
                    tri_fp_chars += len(pred_trigger) - trigger_max_matching_chars
                    tri_fn_chars += len(trigger_true_gold) - trigger_max_matching_chars
                    else_tp_chars += else_max_matching_chars
                    else_fp_chars += len(pred_opi) - else_max_matching_chars
                    else_fn_chars += len(else_true_gold) - else_max_matching_chars
                    sentiment_tp_chars += sentiment_max_matching_chars
                    sentiment_fp_chars += 1 - sentiment_max_matching_chars
                    sentiment_fn_chars += 1 - sentiment_max_matching_chars
                        
        if mode in ['triggers', 'arguments', 'opinions']:
            p = tp_chars / (tp_chars + fp_chars) if tp_chars + fp_chars > 0 else 0
            r = tp_chars / (tp_chars + fn_chars) if tp_chars + fn_chars > 0 else 0
            f1 = 2 * p * r / (p + r) if p + r > 0 else 0
            scores = [p, r, f1]
        elif mode in ['trigger-arg', 'trigger-opinion']:
            p1 = tri_tp_chars / (tri_tp_chars + tri_fp_chars) if tri_tp_chars + tri_fp_chars > 0 else 0
            r1 = tri_tp_chars / (tri_tp_chars + tri_fn_chars) if tri_tp_chars + tri_fn_chars > 0 else 0
            # f1_1 = 2 * p1 * r1 / (p1 + r1) if p1 + r1 > 0 else 0
            p2 = else_tp_chars / (else_tp_chars + else_fp_chars) if else_tp_chars + else_fp_chars > 0 else 0
            r2 = else_tp_chars / (else_tp_chars + else_fn_chars) if else_tp_chars + else_fn_chars > 0 else 0
            ...
            p2 = else_tp_chars / (else_tp_chars + else_fp_chars) if else_tp_chars + else_fp_chars > 0 else 0
            r2 = else_tp_chars / (else_tp_chars + else_fn_chars) if else_tp_chars + else_fn_chars > 0 else 0
            p3 = sentiment_tp_chars / (sentiment_tp_chars + sentiment_fp_chars) if sentiment_tp_chars + sentiment_fp_chars > 0 else 0
            r3 = sentiment_tp_chars / (sentiment_tp_chars + sentiment_fn_chars) if sentiment_tp_chars + sentiment_fn_chars > 0 else 0
            p = (p1 + p2 + p3) / 3
            r = (r1 + r2 + r3) / 3
            f1 = 2 * p * r / (p + r) if p + r > 0 else 0
            scores = [p, r, f1]
    ...
    def count_matching_words_for_english(self, pred, gold):
        # 将元组转换为列表
        pred = list(pred)
        gold = list(gold)
    ...
    def forward(self, print_line=False, lang='zh'):
        ...
        # items = ['triggers', 'arguments', 'opinions', 'trigger-arg', 'trigger-opinion', 'tri_opi_senti_pairs', 'quad', 'iden']
        items = ['triggers', 'arguments', 'opinions', 'trigger-arg', 'trigger-opinion', 'tri_opi_senti_pairs']
        # item_name = ['Triggers', 'Arguments', 'Opinions', 'Tri-Arg', 'Tri-Opi', 'Tri-Opi-Senti', 'quad', 'iden']
