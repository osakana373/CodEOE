...
class MyDataLoader:
    ...
    def collate_fn(self, lst):
        ...
        # new_sentences_ids = padding(new_sentences_ids)
        ...
        # entity_lists, relation_lists = map(padding, [entity_list, relation_list])
        ...
        # nocuda = ['pairs', 'doc_id', 'pieces2words', 'new2old']
...
class RelationMetric:
    ...
    def compute(self, name='valid', epoch=0):
        args = AttrDict({
            'pred_file': os.path.join(self.config.target_dir, 'pred_{}_{}.json'.format(self.config.lang, name, epoch)),
