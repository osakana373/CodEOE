...
from src.common import set_seed, update_config, ScoreManager
...
class Main:
    ...
    def train_iter(self):
        ...
        for i, data in enumerate(train_data):
            ...
            losses.append([l.tolist() for l in loss])
    ...
    def train(self):
        ...
        for epoch in range(self.config.epoch_size):
            ...
            tri_arg_score, tri_opi_senti_score, res = self.relation_metric.compute(epoch=epoch)

            self.model.to(self.config.device)
            if self.config.lang == 'zh':
                with open('zh_valid_score_log.txt', 'a') as f:
                    f.write(str(res))
                    f.write('\n')
            else:
                with open('en_valid_score_log.txt', 'a') as f:
                    f.write(str(res))
                    f.write('\n')
            self.test()

            if tri_arg_score + tri_opi_senti_score > best_score:
                best_score = tri_arg_score + tri_opi_senti_score
                best_iter = epoch
    ...
    def test(self):
        ...
        tri_arg_score, tri_opi_senti_score, res = self.relation_metric.compute('test')

        self.model.to(self.config.device)
        if self.config.lang == 'zh':
            with open('zh_test_score.txt', 'a') as f:
                f.write(str(res))
                f.write('\n')
        else:
            with open('en_test_score.txt', 'a') as f:
                f.write(str(res))
                f.write('\n')
